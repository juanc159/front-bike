...
const { taxes, currentPage, lastPage/* , totalPage, totalData*/, loading  } = storeToRefs(taxesStore);
...
const changeScreen = async (screen: string, obj: object | null = null) => {
  //taxesStore.clearFormulario()
  taxesStore.typeAction = screen
  console.log(obj);
}

//Accion cambio de estado
const changeState = async (object: object, state: number) => {
  //taxesStore.changeState(object, state);
}
// 👉 Computing pagination data
const paginationData = computed(() => {
  /* const firstIndex = taxes.value.length ? ((currentPage.value - 1) * totalPage.value) + 1 : 0
  const lastIndex = taxes.value.length + ((currentPage.value - 1) * totalPage.value) */
  ...
  <div>
    ...
    <VTable class="text-no-wrap">
      ...
      <tbody>
        ...
        <tr v-show="!loading" v-for="(item, index) in taxes" :key="index" style="height: 3.75rem;">
          <td>
