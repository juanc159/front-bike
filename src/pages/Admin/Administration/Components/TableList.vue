...
//  data paginate
const { administrations, totalPage, lastPage, currentPage, totalData, loading, formulario } = storeToRefs(administrationStore)
...
const changeScreen = async (screen: string, userId: number | null = null) => {
  ...
  if (userId) {
    await administrationStore.fetchInfo(userId)
