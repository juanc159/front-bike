...
const { currentPage, typesCreditNote, totalPage, links, totalData,loading,pathExcel } = storeToRefs(creditNoteStore);
const rowPerPage = ref<number>(10);
const searchQuery = ref<string>("");

//Accion cambio de pantalla
const changeScreen = async (screen: string, typeRecInvId: number | null = null) => {
  creditNoteStore.clearFormulario();
  creditNoteStore.typeAction = screen; 
  if (typeRecInvId) {
    await creditNoteStore.fetchInfo(typeRecInvId)
  }else creditNoteStore.clearArrays();
};

const fetchTypesReceiptInvoices = async () => {
  await creditNoteStore.fetchAll({
...
const paginationData = computed(() => {
  const firstIndex = typesCreditNote.value.length
    ? (currentPage.value - 1) * totalPage.value + 1
    : 0;
  const lastIndex =
    typesCreditNote.value.length + (currentPage.value - 1) * totalPage.value;
...
const dowloadExcel = async() => {
  console.log('dowloadExcel',typesCreditNote)
  await creditNoteStore.excel({
    company_id: authentication.company.id,
    searchQuery: searchQuery.value,
  });
...
const deleteData = async (id: number) => {
  Swal.fire({
    ...
    if (result.isConfirmed) {
      await creditNoteStore.fetchDelete(id)
      await fetchTypesReceiptInvoices()
    }
  ...
  <div>
    <VContainer fluid class="d-flex flex-wrap py-4 gap-4">
      ...
      <div class="app-user-search-filter d-flex align-center flex-wrap gap-4">
        ...
        <VBtn color="primary" @click="changeScreen('form')"> Crear nuevo tipo de nota crédito</VBtn>
      </div>
    </VContainer>
    ...
    <VTable class="text-no-wrap">
      ...
      <tbody>
        ...
        <tr
        v-show="!loading"
          v-for="(item, index) in typesCreditNote"
          :key="index"
