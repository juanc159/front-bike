...
// menu data paginate 
const { invoices, totalPage, lastPage, currentPage, totalData, loading,showInvoice,searchInvoice,pathExcel,customers_arrayInfo,customers_countLinks,userSeller_arrayInfo,userSeller_countLinks } = storeToRefs(noteDebitStore)
...
onMounted(async () => {
  if (noteDebitStore.typeAction != "back")
    await fetchInvoice()
})
...
const changeScreen = async (screen: string, invoiceId: number | null = null) => {
  // noteDebitStore.clearFormulario()
  noteDebitStore.typeAction = screen;
  // if (invoiceId)
  //   noteDebitStore.fetchInfoInvoice(invoiceId)
}
...
const deleteData = async (id: number) => {
  Swal.fire({
    ...
    if (result.isConfirmed) {
      await noteDebitStore.fetchDelete(id)
      await fetchInvoice()
    }
...
const selectInfinite = useSelect2Infite(noteDebitStore.fetchCustomerOfThirdList, customers_arrayInfo, {
company_id: authentication.company.id,
});
const selectInfinite1 = useSelect2Infite(noteDebitStore.fetchSellerOfUserList, userSeller_arrayInfo, {
  company_id: authentication.company.id,
});
onMounted(async () => {
