...
const { generals, currentPage, totalPage, lastPage, totalData, loading } = storeToRefs(generalStoreWayToPay);
...
const fetchGeneral = async () => {
  await generalStoreWayToPay.fetchAll({
...
const changeScreen = async (screen: string, obj: object | null = null) => {
  generalStoreWayToPay.clearFormulario()
  generalStoreWayToPay.typeAction = screen
  console.log(obj);
  
  if (obj) {
    const b = generalStoreWayToPay.ledgerAccountAuxiliar.find(ele => ele.id === obj.ledger_account_auxiliary_id)
    if (!b) {
      generalStoreWayToPay.ledgerAccountAuxiliar.push({
    ...
    const bb = generalStoreWayToPay.paymentMethods.find(ele => ele.id === obj.payment_method_id)
    if (!bb) {
      generalStoreWayToPay.paymentMethods.push({
    ...
    if (obj.id)
      generalStoreWayToPay.fetchInfoGeneral(obj.id)
  }
}

//Accion cambio de estado
const changeState = async (object: object, state: number) => {
  generalStoreWayToPay.changeState(object, state);
}

// 👉 Computing pagination data
const paginationData = computed(() => {

  const firstIndex = generals.value.length ? ((currentPage.value - 1) * totalPage.value) + 1 : 0
  const lastIndex = generals.value.length + ((currentPage.value - 1) * totalPage.value)
  ...
  <div>
    ...
    <VTable class="text-no-wrap">
      ...
      <tbody>
        ...
        <tr v-show="!loading" v-for="(item, index) in generals" :key="index" style="height: 3.75rem;">
          <td>
            <v-switch v-model="item.in_use" color="success" inset :value="item.in_use" :true-value="1" :false-value="0"
              hide-details @click="changeState(item, item.in_use)"></v-switch>
          </td>
          <td>
            <span> {{ item.code }} </span>
          </td>
          ...
          <td class="text-center" style="width: 5rem;">
            ...
            <VBtn icon size="x-small" color="default" variant="text" @click="changeScreen('form', item)">
